...
def where_is_it_called(func):
    ...
    def _where_is_it_called(*args, **kwargs):
        ...
        where_is_it_called.log.debug(
            f"文件[{func.__code__.co_filename}]的第[{func.__code__.co_firstlineno}]行即模块 [{func.__module__}] 中的方法 [{func_name}] 正在被文件 [{file_name}] 中的"  # noqa
...
class TimerContextManager:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self._is_print_log:
            self.log.debug(
                f'对下面代码片段进行计时:  \n执行"{self._file_name}:{self._line}" 用时 {round(self.t_spend, 2)} 秒'  # noqa
            )
...
class _Test(unittest.TestCase):
    ...
    def test_exception_context_manager(self):
        ...
        with ExceptionContextManager() as ec:  # noqa
            run()
