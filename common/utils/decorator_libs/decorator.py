    ...
    def _run_many_times(func):
        ...
        def __run_many_times(*args, **kwargs):
            for i in range(times):
                run_times_log.debug(
                    "* " * 50 + f"当前是第 {i + 1} 次运行[ {func.__name__} ]函数"
                )
                func(*args, **kwargs)
...
def handle_exception(
    retry_times=0, error_detail_level=0, is_throw_error=False, time_sleep=0
):
    """捕获函数错误的装饰器,重试并打印日志
            ...
            for i in range(0, retry_times + 1):
                try:
                    ...
                    if error_detail_level == 0:
                        ...
                        error_info = (
                            "错误类型是：" + str(e.__class__) + "  " + traceback.format_exc()
                        )
        ...
        def _cached_function_result_for_a_time(fun):
            ...
            def __cached_function_result_for_a_time(*args, **kwargs):
                ...
                if (
                    fun,
                    key,
                ) in cls.func_result_dict and time.time() - cls.func_result_dict[
                    (fun, key)
                ][
                    1
                ] < cache_time:
...
def deprecated(fn):
    ...
    def wrapper(*args, **kwargs):
        warnings.warn(
            fn.__doc__.split("\n")[0], category=DeprecationWarning, stacklevel=2
        )
...
class RedisDistributedLockContextManager(LoggerMixin):
    ...
    def __enter__(self):
        ...
        self.redis_client.set(
            self.redis_lock_key, value=self.identifier, ex=self._expire_seconds, nx=True
        )
...
def add_cors_according_is_mtfy_app_gw(response):
    ...
    if "mtfy-app-gw" in flask_request.headers.get("X-Forwarded-Host", ""):
        ...
        response.headers["Access-Control-Allow-Origin"] = flask_request.headers.get(
            "Origin", "*"
        )  #
        response.headers["Allow"] = "GET, HEAD, POST, OPTIONS, PUT, PATCH, DELETE"
        # 这里的Access-Control-Allow-Headers使用*会造成老的手机系统不支持通配符。同时option请求才有这个键 Access-Control-Request-Headers，所以用headers.get。  # noqa
...
class _Test(unittest.TestCase):
    ...
    def test_singleton(self):
        ...
        class A:  # noqa
            """
            common/utils/decorator_libs/decorator.py:504:9:
            B903 Data class should either be immutable or use __slots__ to save memory.
            Use collections.namedtuple to generate an immutable class,
            or enumerate the attributes in a __slot__ declaration in the class
            to leave attributes mutable.
            """
